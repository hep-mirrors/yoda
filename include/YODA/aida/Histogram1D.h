...
public:
  ...
  /**
   * Fill the IHistogram1D with a bin index and the
   * corresponding weight.
   * @param index  The bin index to be filled in.
   * @param weight The corresponding weight (by default 1).
   * @return false If the weight is <0 or >1 (?).
   */
  bool fillBin(int index, double weight = 1.) {
    int i = index + 2;
    double x = 0.5*(ax->binLowerEdge(i) + ax->binUpperEdge(i));
    ++sum[i];
    sumw[i] += weight;
    sumxw[i] += x*weight;
    sumx2w[i] += x*x*weight;
